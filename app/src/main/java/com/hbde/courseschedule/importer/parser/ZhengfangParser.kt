 ...
 * 正方教务系统解析器
 *
 * 正方教务系统课表通常包含在 id="kbtable" 的表格中
 * 表格结构：第一行为星期标题，第一列为节次，单元格内使用 div 包裹课程信息
 */
class ZhengfangParser : CourseParser {

    override val parserName: String = "正方教务系统"
    override val systemType: String = "zhengfang"

    override fun canParse(html: String): Boolean {
        return html.contains("kbtable") ||
    ...
    override fun parse(html: String): List<RawCourse> {
        ...
        // 正方系统常见的课表表格选择器
        val table = document.selectFirst("#kbtable, table#kbtable, .kbcontent, table.kb")
    ...
    /**
     * 解析单元格内的课程信息
     * 正方系统通常用 <div class="kbcontent"> 或直接的文本包含课程信息
     */
    private fun parseCell(cell: Element, dayOfWeek: Int, startNode: Int, endNode: Int): List<RawCourse> {
        // 正方系统常见结构：单元格内有多个 div，每个 div 是一门课
        val contentDivs = cell.select("div.kbcontent, div[class*='kb']")
        if (contentDivs.isNotEmpty()) {
            return contentDivs.flatMap { div ->
