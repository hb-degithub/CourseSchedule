 ...
 * 强智教务系统解析器
 *
 * 强智教务系统课表通常包含在 class="kb" 或 class="table" 的表格中
 * 表格结构：第一行为星期标题，第一列为节次，单元格内可能包含多门课程
 */
class QiangzhiParser : CourseParser {

    override val parserName: String = "强智教务系统"
    override val systemType: String = "qiangzhi"

    override fun canParse(html: String): Boolean {
        return html.contains("强智") ||
               html.contains("qiangzhi") ||
               html.contains("kb-table") ||
               html.contains("class=\"kb\"")
    }
    ...
    override fun parse(html: String): List<RawCourse> {
        ...
        // 强智系统常见的课表表格选择器
        val table = document.selectFirst("table.kb, table.kb-table, .kb table, table[border='1']")
    ...
    /**
     * 解析单元格内的课程信息
     * 强智系统一个单元格可能包含多门课程，用 <br> 或 <div> 分隔
     */
    private fun parseCell(cell: Element, dayOfWeek: Int, startNode: Int, endNode: Int): List<RawCourse> {
        val text = cell.html()
        // 按 <br> 或 <br/> 分割多个课程
        val courseBlocks = text.split(Regex("<br\\s*/?>", RegexOption.IGNORE_CASE))
        ...
        if (courseBlocks.isEmpty()) {
            ...
            if (divs.isNotEmpty()) {
                return divs.flatMap { parseCourseBlock(it.text(), dayOfWeek, startNode, endNode) }
            }
        }

        return courseBlocks.flatMap { block ->
    ...
    private fun parseNodeRange(nodeText: String, rowIndex: Int): Pair<Int, Int> {
        ...
        // 默认：第 n 行对应第 n 节课（每节课 1 节）
        return Pair(rowIndex, rowIndex + 1)
    }
